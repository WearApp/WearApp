...
@interface LFindLocationViewController ()<UITableViewDataSource,UITableViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UISearchBarDelegate,CLLocationManagerDelegate, SKAMapLocationDelegate>
{
    NSArray *provinces;
    NSArray *cities;
    NSMutableArray *arr;
...
//    [leftNavBtn setTitle:@"退" forState:UIControlStateNormal];
    [leftNavBtn setImage:[UIImage imageNamed:@"down"] forState:UIControlStateNormal];
    [leftNavBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [leftNavBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    ...
    searchBar.searchTextFieldBackgroundColor = [UIColor lightGrayColor];
...
- (void)viewDidLoad {
    ...
    [self reloadLocationWithAMapKit];
    
    [whereBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
    [whereBtn setTitle:@"正在定位，请稍等..." forState:UIControlStateNormal];
    
    // todo
    
    provinces = [[NSArray alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"APIStoreCityList.plist" ofType:nil]];
...
//                [whereBtn setTitle:@"定位失败，请重新定位" forState:UIControlStateNormal];
            ...
            tapBtn.frame = CGRectMake(self.view.frame.size.width-100, 0, 45, 40);
            ...
            [tapBtn setTitle:@"重定位" forState:UIControlStateNormal];
            [tapBtn addTarget:self action:@selector(reloadLocationWithAMapKit) forControlEvents:UIControlEventTouchUpInside];
//            [tapBtn addTarget:self action:@selector(reLOadLoction) forControlEvents:UIControlEventTouchUpInside];
    ...
    else
    {
        ...
        UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 40)];
        titleView.backgroundColor = [UIColor colorWithWhite:0.940 alpha:1.000];
...
//        [cell setBackgroundColor:[UIColor clearColor]];
        if (indexPath.section == 0) {
            if (indexPath.row == 0) {
                cell.backgroundColor = [UIColor colorWithWhite:0.940 alpha:1.000];
...
- (void)reloadLocationWithAMapKit {
...
- (void)didGetProvince:(NSString *)province city:(NSString *)city district:(NSString *)district {
    ...
    NSDictionary *cityDictionary = [[NSDictionary alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"cityID" ofType:@"plist"]];
    NSString *cityName = cityDictionary[areaidDictionary[city]];
    
    whereStr = cityName;
    [whereBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [whereBtn setTitle:whereStr forState:UIControlStateNormal];
    whereBtn.userInteractionEnabled = YES;
    
    self.loctionCity = cityName;
    
    NSLog(@"cityname%@",cityName);
    
    [[NSUserDefaults standardUserDefaults] setObject:cityName forKey:@"USER_WHERE"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (void)didFailLocationWithError:(NSError *)error {
    if ([SKAMapLocation shareManager].locatedSuccess) {
        return;
    }
    
    whereStr = [NSString stringWithFormat:@"定位失败，错误代码：%li", (long)error.code];
    [whereBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    [whereBtn setTitle:whereStr forState:UIControlStateNormal];
    whereBtn.userInteractionEnabled = YES;
    
    [[NSUserDefaults standardUserDefaults] setObject:@"ERROR" forKey:@"USER_WHERE"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
- (void)didGetLocalAreaID:(NSString *)areaid cityCode:(NSString *)cityCode districtCode:(NSString *)district {
